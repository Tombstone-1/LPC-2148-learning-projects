...
int main(){
	...
	// config keypad p16 -19 input, p20 -24 output.
	IO1DIR = 0x00F00000;
...
char keypad(void){
	while(1){
		...
		if(!(IO1PIN & (1<<19))){ while(!(IO1PIN & (1<<19))); return '/';}
		
		// FOR ROW 2 SCAN
		IO1CLR = (1<<21);
		IO1SET = (1<<20) | (1<<22) | (1<<23);
		...
		if(!(IO1PIN & (1<<19))){ while(!(IO1PIN & (1<<19))); return 'x';}
		
		// FOR ROW 3 SCAN
		IO1CLR = (1<<22);
		IO1SET = (1<<20) | (1<<21) | (1<<23);
		...
		if(!(IO1PIN & (1<<16))){ while(!(IO1PIN & (1<<16))); return 'C';}
